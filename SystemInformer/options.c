    ...
    )
{
    ...
    if (!ReloadOnly)
    {
        ...
        PhAddListViewItem(listViewHandle, PHP_OPTIONS_INDEX_START_HIDDEN, L"启动后隐藏", NULL);
        ...
        PhAddListViewItem(listViewHandle, PHP_OPTIONS_INDEX_ENABLE_NETWORK_RESOLVE_DOH, L"通过 HTTPS解析 DNS (DoH) ", NULL);
        PhAddListViewItem(listViewHandle, PHP_OPTIONS_INDEX_ENABLE_INSTANT_TOOLTIPS, L"立即显示工具提示", NULL);
        PhAddListViewItem(listViewHandle, PHP_OPTIONS_INDEX_ENABLE_IMAGE_COHERENCY, L"检查镜像的一致性", NULL);
        PhAddListViewItem(listViewHandle, PHP_OPTIONS_INDEX_ENABLE_STAGE2, L"检查图像中的数字签名", NULL);
        ...
        PhAddListViewItem(listViewHandle, PHP_OPTIONS_INDEX_PROPAGATE_CPU_USAGE, L"包括折叠进程的使用", NULL);
